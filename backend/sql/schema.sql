...
CREATE TABLE `wecom_inbound_event` (
    ...
    `turn_id`            CHAR(36)        NOT NULL                     COMMENT '不可变入站轮次 ID；重试复用，人工重放新建',
    ...
    `rate_limit_decision` ENUM('accepted','rate_limited') NOT NULL DEFAULT 'accepted' COMMENT '限流审计决策',
    `rate_limit_rule`    VARCHAR(128)    DEFAULT NULL                 COMMENT '限流规则/版本（仅审计）',
    `rate_limited_at`    DATETIME(6)     DEFAULT NULL                 COMMENT '限流决策时间',
    `dispatcher_lease_owner` VARCHAR(64) DEFAULT NULL                 COMMENT '入站 dispatcher 当前 owner',
    `dispatcher_lease_expires_at` DATETIME(6) DEFAULT NULL             COMMENT '入站 dispatcher lease 到期时间',
    ...
    KEY `idx_inbound_dispatch` (`status`, `rate_limit_decision`, `created_at`, `id`),
    KEY `idx_inbound_dispatch_lease` (`status`, `rate_limit_decision`, `dispatcher_lease_expires_at`, `id`),
...
-- ============================================================================
-- 12. action_execution Action 幂等执行凭据（Job Search v1）
-- ============================================================================
...
-- ============================================================================
-- 13. wecom_outbound_outbox 企微回复事务出站箱
-- ============================================================================
