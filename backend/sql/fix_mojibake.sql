...
--
-- 安全机制：
--   1. 只更新"看起来像 mojibake"的行（WHERE col LIKE '%ç%' OR '%å%' OR '%æ%' …）
--      这几个字符是 UTF-8 Chinese 首字节被 Latin-1 解码的必然前缀。
--   2. 用 START TRANSACTION — 确认无误再 COMMIT，不对就 ROLLBACK。
--
-- 用法（在 docker 环境）：
...
--
-- 或用配套的 scripts/fix_mojibake.sh，带 dry-run 和确认步骤。
-- ============================================================================
...
-- 查看修复后的样本，确认无误再 COMMIT；如果坏了请改成 ROLLBACK。
SELECT '=== user 样本 ===' AS _;
...
COMMIT;
-- 如需回滚：把上面的 COMMIT 改成 ROLLBACK，整个脚本重新灌一次。
