...
### 阶段四 PR3 接管现状（2026-05-02）

**primary 模式入口已接通**（intent_service.classify_dialogue 增加 primary 分支 +
`_is_primary_rollout_target` 灰度桶判定）。生产 `dialogue_v2_mode=primary` +
`dialogue_policy.primary_rollout_percentage` 控制灰度比例（5/25/50/100% 节奏，
plan §4.1.6）。

primary 路径与 dual_read 共用 v2 派生链路（`_classify_dialogue_v2` helper），
HIGH 项 cancel / proceed / resume 在 primary 路径下同样由 `dialogue_act` /
`resolve_conflict + conflict_action` 接管。

**关键词词表当前状态（PR3 后）**：

- ✅ `_CANCEL_FULL` / `_CANCEL_PREFIX` / `_is_cancel`：保留作 fallback。
  v2 主路径（dual_read / primary）由 LLM `dialogue_act=cancel` 接管；
  legacy fallback / shadow 路径继续依赖关键词。
- ✅ `_PROCEED_KEYWORDS` / `_RESUME_KEYWORDS`：保留作 `upload_conflict` 状态下
  闭集选项的解析兜底（产品给定有限词集，不属「关键词膨胀」）。
- ✅ 斜杠命令（`/取消` `/帮助` `/重新找`）：闭集，确定性解析，长期保留。
- ✅ `_WORKER_SEARCH_SIGNALS` / `_JOB_POSTING_SIGNALS`：PR1 下沉到
  `_apply_worker_intent_guard`，仅 legacy 路径调用，与 v2 主链路解耦。
- ❌ `_CITY_ADD_SIGNALS` / `_CITY_REPLACE_SIGNALS`：PR1 已删除（0 引用死代码）。

**`criteria_patch` 收口（PR3）**：

- LLM prompt v2.7 起，所有 intent 一律输出 `criteria_patch: []`（不再生成 op 语义）。
- `IntentResult.criteria_patch` 字段保留 schema 向后兼容（旧 provider / legacy 序列化）。
- v2 派生路径（`dialogue_compat.decision_to_intent_result`）天然输出空 patch;
  `merge_criteria_patch` 接收空 list 是 no-op，无副作用。
- legacy 路径仍兼容 `merge_criteria_patch`（结构性保留），但新 prompt 引导 LLM 不输出 op。

**还在 plan 中、留待阶段五的清理**：

- 移除 `_LEGACY_DIALOGUE_FIELD_MAP` 旧 env 兼容层（PR2 引入，阶段五 dual_read
  完全下线后删除）。
- 移除 `_classify_intent_legacy` 内 worker 护栏（v2 占 100% 后没必要再保留 fallback 词表）。
