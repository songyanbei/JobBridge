...
- 结果：63 个已知问题均使用独立提交完成修复；`d13ec5e` 的 V-01 至 V-05 全量证据继续有效，之后十三项收口修复在最终代码 HEAD `9780cc3` 上通过完整单元测试及对应的真实 MySQL/Redis 集成门禁，独立评审未发现未关闭的 P1/P2/P3。Stage A 旧 schema 兼容冒烟是前次最终验收证据；页面联调是 `6198fe7` 上的历史补充证据，不声明已在最终 HEAD 重跑。
...
| 56 | H-06 down 门禁未拒绝 inbound CHECK/FOREIGN KEY | `a10252c` | CHECK/FK 真实 MySQL 写入失败负例、独立复审 |
| 57 | H-07 down 门禁遗漏任意命名的 inbound 触发器 | `c7cbd3f` | 任意命名触发器真实 MySQL 写入失败负例、独立复审 |
| 58 | H-08 down 门禁误放行额外非唯一函数索引 | `7e55b35` | 函数/前缀等不安全索引真实 MySQL 负例、独立复审 |
| 59 | H-09 down 门禁未校验 job 事务引擎 | `5329b67` | Job MyISAM 回滚失效真实 MySQL 负例、独立复审 |
| 60 | H-10 down 未建立完整 Job Stage A 结构合同 | `9fde5e8` | 完整 45 列/索引/约束/表属性真实 MySQL 负例、独立复审 |
| 61 | H-11 down 遗漏引用 inbound 的反向外键 | `b878273` | 同库/跨库反向 FK、受限元数据账号及 1451 真实 MySQL 负例、独立复审 |
| 62 | H-12 down trigger 扫描因权限 fail-open | `82a797e` | 账号绑定、隐藏 trigger 与 1644 真实 MySQL 负例、独立复审 |
| 63 | H-13 down 误放行重复备份行 | `9780cc3` | 缺失主键、重复 job_id 和行数差真实 MySQL 负例、独立复审 |
...
以下页面联调执行于 2026-08-14、代码 HEAD `6198fe7`，用于确认 B-01 至 B-03 的端到端体验；它不是最终 HEAD `9780cc3` 的页面重跑证据：
...
`6198fe7` 之后的 R-01 至 R-12、T-01、F-01 至 F-07 和 G-01 至 G-04 共 24 个追踪问题，分别由对应提交的定向测试、真实 MySQL/Redis 门禁，以及 `d13ec5e` 上的 V-01 `1997 passed`、主集成 `103 passed`、停机 `1 passed` 和补充 Redis `5 passed` 覆盖。Stage A `1 passed` 为前次验收证据；由于本轮未在 `9780cc3` 重跑完整页面流程，本报告不将历史页面联调作为最终 HEAD 的合并门禁依据。
...
H-06 至 H-09 完成后，在最终代码 HEAD `5329b67` 再次重跑相同收口门禁，结果为完整单元 `2003 passed`、完整 Redis/MySQL 集成 `56 passed`、完整 destructive-down MySQL `4 passed`。新增真实 MySQL 负例覆盖未声明 CHECK、FOREIGN KEY、任意命名的 `BEFORE INSERT` 触发器、非唯一函数索引，以及 Job 使用 MyISAM 时事务回滚失效；额外普通非唯一物理列 BTREE 索引继续按发布策略允许。四项均在本项门禁通过和独立只读评审无 P1/P2/P3 后分别提交。

H-10 至 H-13 完成后，在最终代码 HEAD `9780cc3` 重跑完整收口门禁：完整单元 `2003 passed`、完整 Redis/MySQL 集成 `56 passed`、完整 destructive-down MySQL `6 passed`。新增真实 MySQL 负例覆盖 Job 的完整 45 列、精确索引/约束/表属性合同，跨 schema 反向外键与受限元数据账号，缺少 TRIGGER 权限时不可见的 root trigger，以及删除备份主键后复制完全相同的 `job_id` 行；rollout 单测另行覆盖 SessionLocal 账号与期望迁移账号不一致时 fail-closed。四项均在修复独立评审意见、重跑全部本项门禁并再次取得无 P1/P2/P3 结论后独立提交。
