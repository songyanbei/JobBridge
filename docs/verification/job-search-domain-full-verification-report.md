...
本轮不能正式结项。工程定向集合和当前源码链路通过，但生产开关仍 fail-closed，页面使用的 Docker app/worker 是无源码挂载的旧镜像，导致页面首发岗位出现 `version=2`、`aggregate_version=1`。已提交最小加固和回归测试；重建/重启镜像后必须重新执行页面 GF1。
...
- 隔离身份 inbound 两条均 `status=done`、`retry_count=0`；页面新岗位为 `job.id=35`、`audit_status=passed`、`activated_at/expires_at` 已写入，但旧镜像写出 `version=2`、`aggregate_version=1`。
- 同一库历史 seed 岗位也存在 `version=2`、`aggregate_version=1`，证明旧镜像/历史迁移数据不满足双写约束。
- 当前源码直连 MySQL 重放生成 `job.id=36`，两个版本及 domain event 版本均为 2，证明加固后源码路径正确。
- 页面 SSE 显示连接成功并收到 bot 帧；`/inbound` 返回 200。Contact 因默认 `contact_service_mode=off` 未创建 grant/delivery/outbox，页面提示为预期 feature-off。

GF1 判定：页面旧镜像路径 `BLOCKED/P1`；当前源码路径 `PASS`；Contact 端到端 `SKIPPED`（生产开关 off）。按用户指示，未继续 GF2-GF4 页面扩展。

### FAIL / BLOCKED

1. **P1 版本双写（已加固）**：旧 Docker app/worker 无源码挂载，页面首发仍为 `version=2/aggregate_version=1`。必须重建镜像并重启 app、worker 后复测。提交 `686e93f` 已显式初始化两个版本并增加回归测试。
...
8. **四条页面 flow**：GF2/GF3/GF4 未执行（按阻断优先指示停止）；不能宣称四流通过。
9. **ContactDelivery**：未在页面执行 `authorize -> issue grant -> redeem -> ContactDelivery`，因为默认 Contact off；对应真实 Contact 单测/混沌矩阵通过，但页面级项 SKIPPED。
...
1. 构建并部署包含 `686e93f` 的 app/worker 镜像，确认容器不再使用无源码挂载旧层；重跑隔离 GF1 并核对两个版本及 `job.published` event。
...
**不可正式结项（BLOCKED）**。当前源码首发版本一致性已通过最小修复验证，但页面旧镜像仍可产生 P1 数据不一致；四条页面 golden flow 未完成，Contact on/生产 rollout/长期观察也未完成。 
