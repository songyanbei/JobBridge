...
- [ ] FAIL J4 manifest LF/SHA 已修复（`test_manifest_pins...` `1 passed`）；root DSN 下 Phase11 checkpoint 参数化 `4 passed`、stage-2 activation `1 passed`，但完整五文件集合运行约 7 分钟后超时中止，既有真实集成基线仍为 `20 passed, 13 failed, 103 skipped`，`jobbridge` 测试账号无临时 schema 权限。
...
| J4 | manifest contract；`RUN_INTEGRATION=1 PHASE11_TEST_MYSQL_DSN=mysql+pymysql://root:root@127.0.0.1:3306/jobbridge PHASE11_TEST_REDIS_DSN=redis://127.0.0.1:6379/0 pytest -q tests/integration/test_phase11_stage1_migration_mysql.py::test_python_batch_checkpoint_survives_process_crash_with_full_run_audit`; `test_phase11_stage2_activation_mysql.py`；完整五文件集合 | WSL `.venv-wsl`/MySQL/Redis | manifest `1 passed`；checkpoint `4 passed`；activation `1 passed`；完整集合超时中止；历史集成 `20 passed,13 failed,103 skipped` | 未取得完整集合最终摘要；历史失败原因为 `jobbridge` 无 CREATE 临时 schema 权限 | 使用专用测试账号并重新跑完整集合 |
